...
df <- veracruz
p1 <- ggplot(df, aes(x = X, y = Searches)) +
  ylab("Searches") +
  xlab("") +
  geom_line() +
...
####################
require(forecast)
fit_cons_busquedas <- auto.arima(df$Cases, xreg = df$Searches)
fcast_busquedas <- c(42, 42) #Busquedas siguientes
